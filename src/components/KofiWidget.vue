<template>
  <div class="group fixed bottom-5 right-5 z-50">
    ...
    <div
        class="absolute bottom-full right-0 mb-2 w-56 rounded-lg bg-gray-800 px-3 py-2 text-center text-sm text-white opacity-0 transition-opacity group-hover:opacity-100 pointer-events-none"
    >
      Enjoying the site? Consider supporting to help with upkeep!
      <div class="absolute right-6 top-full h-0 w-0 -translate-x-1/2 border-x-8 border-t-8 border-x-transparent border-t-gray-800"></div>
    </div>
  </div>
</template>
